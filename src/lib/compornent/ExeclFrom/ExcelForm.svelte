<script lang="ts">
  ...
  import CourseExcelPreview from "./CourseExcelPreview.svelte";
  ...
  let previewRows: ReadTableRow[] = [];

  const parseResponseRows = (response: any): ReadTableRow[] => {
  ...
  const handleConfirm = async () => {
    ...
    try {
        ...
        previewRows = parseResponseRows(response);
        isPreviewOpen = true;
        errorMessage = "";
    }
    catch (err:any){
...
{#if isPreviewOpen}
  <CourseExcelPreview rows={previewRows} onClose={close} onBack={resetPreview} />
{:else}
  <div class="w-[507px] h-[507px] bg-[#FCFAF7] rounded-4xl p-[38px] shadow-xl flex flex-col justify-between select-none box-border text-left my-font">
    <div>
              ...
              <div class="flex items-center justify-between bg-white/70 p-2.5 rounded-xl border border-[#FD6F02]/20 shadow-sm">
                <div class="flex items-center gap-2.5 overflow-hidden">
                  <svg 
                    ...
                    stroke-linecap="round" 
                    stroke-linejoin="round" 
                    class="w-5 h-5 text-[#FD6F02] shrink-0"
                  >
